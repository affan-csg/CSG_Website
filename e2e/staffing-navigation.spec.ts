...
test.describe("Specialized Roles Navigation E2E Tests", () => {
  ...
  test("should handle invalid specialty slug gracefully", async ({ page }) => {
    ...
    await page.goto("/staffing/non-existent-specialty-12345", { waitUntil: "load" });
    ...
    const isOnHome =
      pageStatus ===
      page.context().browser()?.contexts()[0].pages()[0].url().split("/").slice(0, 3).join("/");

    expect(isOnSpecialty || isOnStaffingDir || isOnHome || pageStatus.endsWith("/")).toBe(true);
  });
  ...
  test("should display related specialties or CTA to other specialties", async ({ page }) => {
    ...
    const relatedSection = page.locator("text=/related|other practices|similar|explore/i");
...
test.describe("Staffing Hub Navigation E2E Tests", () => {
  ...
  test("legacy /pods URL should 301 redirect to /staffing/pods", async ({ page }) => {
    const response = await page.goto("/pods", { waitUntil: "load" });
