...
test.describe("Regional Global Delivery E2E Tests", () => {
  test("should load global-delivery main page", async ({ page }) => {
    ...
    // Verify page title
    await expect(page).toHaveTitle(/US vs LATAM vs Pakistan/i);
  ...
  test("should navigate to US region page", async ({ page }) => {
    ...
    // Look for US region link (scoped to main to avoid the footer's duplicate link)
    const usLink = page.locator('main a[href*="/global-delivery/us"]');
  ...
  test("should navigate to LATAM region page", async ({ page }) => {
    ...
    // Look for LATAM region link (scoped to main to avoid the footer's duplicate link)
    const latamLink = page.locator(
      'main a[href*="/global-delivery/latam"], main a[href*="/global-delivery/latin"]',
    );
  ...
  test("should navigate to Pakistan region page", async ({ page }) => {
    ...
    // Look for Pakistan region link (scoped to main to avoid the footer's duplicate link)
    const pkLink = page.locator(
      'main a[href*="/global-delivery/pakistan"], main a[href*="/global-delivery/pk"]',
    );
  ...
  test("should display pricing or cost information by region", async ({ page }) => {
    ...
    for (const region of regions) {
      ...
      if (region !== "us") {
        ...
        await costSavingsInfo.isVisible().catch(() => false);
  ...
  test("should navigate between regions using browser back/forward", async ({ page }) => {
    ...
    // Go back to LATAM. These are all full page.goto() navigations above, but
    // history back/forward re-hydrates client-side without firing a fresh
    // "load" event — wait on the URL itself instead of the load state.
    await page.goBack();
    await page.waitForURL(/\/global-delivery\/latam/);

    expect(page.url()).toContain("/global-delivery/latam");

    // Go back to US
    await page.goBack();
    await page.waitForURL(/\/global-delivery\/us/);
  ...
  test("should display team or talent info for each region", async ({ page }) => {
    ...
    for (const region of regions) {
      ...
      const teamInfo = page
        .locator("text=/team|talent|developers|engineers|experts|specialists/i")
        .first();

      // Should have some indication of available talent
      await teamInfo.isVisible().catch(() => false);
  ...
  test("should have CTA or contact options on region pages", async ({ page }) => {
    ...
    for (const region of regions) {
      ...
      const ctaButton = page
        .locator('button, a[href*="/contact"], a[href*="/get-started"]')
        .first();
